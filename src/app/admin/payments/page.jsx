...
export default function PaymentsPage() {
  ...
  return (
    <main className="px-6 md:px-10 py-7 max-w-350" style={{ color: token.ink }}>
      {/* Header */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        {kpis.map((k) => (
          <div key={k.label} className="rounded-2xl p-5 border" style={{ background: token.card, borderColor: token.line }}>
            ...
            <p className="font-display font-semibold text-[26px] mt-2" style={{ color: token.ink }}>{loading ? "—" : k.value}</p>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ background: token.card, borderColor: token.line }}>
        <div className="flex flex-wrap items-center justify-between gap-3 px-5 pt-5 pb-3">
          ...
          <div className="flex items-center gap-1.5 p-1 rounded-xl" style={{ background: token.cream }}>
            <Filter size={13} style={{ color: token.inkSoft, marginLeft: 6 }} />
            {FILTERS.map((f) => {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ color: token.inkSoft }} className="text-xs uppercase tracking-wide font-semibold">
                ...
                <td className="px-5 py-2">Reference</td>
            ...
            <tbody>
              ...
              {filtered.map((b) => (
                <tr key={b._id} style={{ borderTop: `1px solid ${token.line}` }}>
                  ...
                  <td className="px-5 py-3 font-medium" style={{ color: token.ink }}>
